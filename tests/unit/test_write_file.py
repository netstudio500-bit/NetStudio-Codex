...
@pytest.mark.asyncio
@pytest.mark.parametrize("arguments", [{}, {"path": 1, "content": "x"}, {"path": "", "content": "x"}, {"path": "x"}, {"path": "x", "content": 1}, {"path": "x", "content": "x", "create": 1}, {"path": "x", "content": "x", "overwrite": 0}, {"path": "x", "content": "x", "extra": True}])
...
async def test_create_and_explicit_intent_rules(tmp_path: Path) -> None:
    ...
    failure(await tool.execute({"path": "new.txt", "content": "bad", "create": True}), "overwrite_not_allowed")
...
async def test_byte_limit_uses_utf8_bytes(tmp_path: Path) -> None:
    ...
    failure(await tool.execute({"path": "x.txt", "content": "éé", "create": True}), "content_too_large")
...
async def test_path_parent_directory_and_reserved_area_controls(tmp_path: Path) -> None:
    ...
    tool = WriteFileTool(tmp_path)
    failure(await tool.execute({"path": "../external-write.txt", "content": "x", "create": True}), "path_outside_workspace")
    failure(await tool.execute({"path": str(external.resolve()), "content": "x", "create": True}), "path_outside_workspace")
    failure(await tool.execute({"path": "missing/x.txt", "content": "x", "create": True}), "parent_not_found")
    (tmp_path / "folder").mkdir()
    failure(await tool.execute({"path": "folder", "content": "x", "overwrite": True}), "not_a_file")
    failure(await tool.execute({"path": ".netstudio/user.txt", "content": "x", "create": True}), "reserved_internal_path")
...
async def test_external_symlink_is_not_writable(tmp_path: Path) -> None:
    ...
    try:
        ...
        raise
    result = await WriteFileTool(tmp_path).execute({"path": "link.txt", "content": "BAD", "overwrite": True})
    failure(result, "path_outside_workspace")
    assert external.read_text(encoding="utf-8") == "SECRET"


@pytest.mark.asyncio
async def test_atomic_replace_failure_preserves_original_and_cleans_temp(tmp_path: Path, monkeypatch) -> None:
    target = tmp_path / "target.txt"
    target.write_text("ORIGINAL", encoding="utf-8")
    monkeypatch.setattr("netstudio.tools.write_file.os.replace", lambda *_: (_ for _ in ()).throw(OSError("replace failed")))
    result = await WriteFileTool(tmp_path).execute({"path": "target.txt", "content": "NEW", "overwrite": True})
...
async def test_diff_truncation_does_not_revert_write(tmp_path: Path) -> None:
    result = await WriteFileTool(tmp_path, max_diff_chars=10).execute({"path": "x.txt", "content": "long content", "create": True})
...
async def test_internal_area_hidden_from_all_model_file_tools(tmp_path: Path) -> None:
    ...
    write = await WriteFileTool(tmp_path).execute({"path": ".netstudio/user.txt", "content": "x", "create": True})
...
def registry(tmp_path: Path) -> ToolRegistry:
    value = ToolRegistry()
    for tool in [ShellTool(tmp_path), ReadFileTool(tmp_path), ListFilesTool(tmp_path), SearchTextTool(tmp_path), WriteFileTool(tmp_path)]:
        value.register(tool)
    return value


def context(tmp_path: Path, *capabilities: ToolCapability) -> ExecutionContext:
    return ExecutionContext(scope=ScopeRef("test", workspace_root=tmp_path), policy=PolicySnapshot(frozenset(capabilities)))
...
def test_registry_keeps_read_and_write_separate(tmp_path: Path) -> None:
    ...
    assert value.view(context(tmp_path, ToolCapability.READ)).names() == ("read_file", "list_files", "search_text")
    assert value.view(context(tmp_path, ToolCapability.READ, ToolCapability.WRITE)).names() == ("read_file", "list_files", "search_text", "write_file")
